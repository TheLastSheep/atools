...
try {
  ...
  assert.equal(mod.homeCommandFallbackIcon(command({ code: "calc" })), "plugin");
  ...
  assert.match(homePanel, /\.recent-icon\s*\{[\s\S]*?width:\s*32px;[\s\S]*?height:\s*32px;/);

  assertSmokeChecked(
    "最近使用磁贴优先显示插件原图标；无图标时显示来源类型 SVG fallback，图标外层约 32px，且网格无横向溢出。",
