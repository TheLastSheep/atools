...
assert.match(workflow, /helperArchitecture:\s*arm64/);
assert.match(workflow, /helperArchitecture:\s*x86_64/);
assert.match(workflow, /find "\$app_path\/Contents" -type f -name 'pasteboard-vision'/);
assert.match(workflow, /test -x "\$helper_path"/);
assert.match(workflow, /codesign --verify --strict "\$helper_path"/);
