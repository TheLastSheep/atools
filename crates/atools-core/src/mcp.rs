...
fn get_builtin_prompt_response(
    ...
    let task_line = task
        ...
        .unwrap_or_default();
    let text = format!(
        "Use ATools MCP tools for local-first work. Prefer pasteboard_search_history and pasteboard_list_pinboards for PasteboardPro discovery; use pasteboard_get_item for redacted structural metadata about one item. Use pasteboard_create_pinboard, pasteboard_update_pinboard, pasteboard_move_pinboard, pasteboard_assign_items, and pasteboard_delete_pinboard only after the normal ClipboardWrite permission check. Use pasteboard_recognize_item for local OCR of a managed PasteboardPro image after ClipboardRead and Screenshot permission checks; its recognized body stays in local searchable history and is redacted from Agent output. Use pasteboard_copy_item after ClipboardRead and ClipboardWrite permission checks; use pasteboard_paste_item only after ClipboardRead, ClipboardWrite, and Shell permission checks because it also sends Command-V. Use pasteboard_sync_status for local redacted vault state; use pasteboard_sync_vault only for a configured encrypted WebDAV vault and after ClipboardRead, ClipboardWrite, and Network permission checks. Prefer find_local_files for filename search, get_current_context for browser/Finder context, rename_files with dry_run for rename planning, compress_images for image size work, ocr_image for explicit local file OCR, open_or_reveal_path for explicit user-approved reveal/open actions, and ask_ai_model only when the user has configured an Agent AI model.{task_line}"
    );
