...
### 2.1 2026-07-22 架构方向调整

ATools 不再以“使用 Rust 完整重写并替代 ZTools”作为目标，正式采用：

> **Rust 核心 + ZTools 生态兼容 + Rust/Node/Web 多运行时插件。**

- Rust 负责高频、性能敏感、安全敏感和本地系统能力。
- ZTools 插件格式、市场与开发者生态是优先兼容和复用对象，不建设第二套封闭生态。
- 新插件可以选择 Rust、Node 或 Web 运行时，但必须注册为统一 `Capability`，进入同一权限、`TaskRun`、Artifact、审计与结果管线。
- Rust 与 Node 插件默认以隔离侧车进程运行，通过 stdio JSON-RPC 或 MCP 通信；不得把不受信任动态库直接加载进主进程。
- Web 与 ZTools 插件运行在隔离 WebView/JavaScript 环境，通过受权限控制的兼容桥访问宿主能力。
- MCP/AI 是能力入口，不是独立实现分支；Agent 与人类入口不得形成两套插件执行逻辑。
...
- 直接兼容 ZTools 插件目录、安装包、市场元数据与发布产物，优先复用其生态。
- Rust、Node、Web 三类原生 ATools 插件的声明、隔离运行、权限和生命周期管理。
...
- 不以插件数量替代性能、兼容性和结果质量。
- 不复制 ZTools 的账号、评论、插件市场服务端和发布社区；仅实现必要的兼容客户端、缓存、安全校验和本地治理。
- 不要求现有 ZTools 插件改写为 Rust；只有性能、安全或系统集成收益明确时才下沉 Rust。
...
插件运行时是实现细节，不能改变上层调用契约：

| 插件类型 | 默认隔离方式 | 推荐协议 | 主要用途 |
| --- | --- | --- | --- |
| Rust | 受控侧车进程 | MCP stdio / JSON-RPC stdio | 系统能力、批处理、性能敏感任务 |
| Node | 受控侧车进程 | MCP stdio / JSON-RPC stdio | 复用 npm 生态、开发工具和自动化 |
| Web | 隔离 WebView | Host Bridge | 交互式 UI、纯前端工具 |
| ZTools 兼容 | 隔离 WebView + 兼容桥 | ZTools API Bridge | 直接复用现有插件生态 |

所有类型必须提供稳定标识、输入/输出 schema、权限 scope、可用性、版本和执行器元数据。面向 MCP/AI 的工具描述必须结构化、可取消、可审计，并尽可能返回结构化内容与 Artifact 引用，而不是只返回自由文本。
...
flowchart TB
    ...
    subgraph ExecutionLayer["执行层"]
        BuiltinTools["Rust 核心工具"]
        RustRuntime["Rust 插件侧车"]
        NodeRuntime["Node 插件侧车"]
        WebRuntime["Web 插件宿主"]
        ZToolsRuntime["ZTools 兼容宿主"]
        NativeBridge["系统原生桥接"]
        ExternalMCP["外部 MCP"]
    ...
    PermissionEngine --> RustRuntime
    PermissionEngine --> NodeRuntime
    PermissionEngine --> WebRuntime
    PermissionEngine --> ZToolsRuntime
    ...
    RustRuntime --> PluginStore
    NodeRuntime --> PluginStore
    WebRuntime --> PluginStore
    ZToolsRuntime --> PluginStore
