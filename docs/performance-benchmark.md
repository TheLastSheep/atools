...
| 单插件内存增量/冷启动 | 内置轻插件冷启动 CI 可追溯，内存增量待测 | 待测 | 待测 | 同一插件夹具、同一启动路径 |
| 连续唤醒与长稳 | 待测 | 待测 | 待测 | 固定轮次和持续时长 |
| 本地数据库增长 | CI 已验证，待同机 | 待测 | 待测 | 固定任务与历史写入负载 |
...
2026-07-14 的 schema v2 运行时证据 [CI run 29343263011](https://github.com/TheLastSheep/atools/actions/runs/29343263011) 全部成功。20 次独立启动的 RSS P99 为 113.625 MiB，首个前端交互报告 P99 为 8397 ms；第 21 个独立进程完成 release smoke 后继续存活 300 秒，空闲 RSS 点样本为 98.016 MiB，CPU 点样本和完整配置保存在 `macos-runtime-performance-28692c78...` artifact。`.app` 目录为 32.71 MiB。首报告仍超过 5 秒软线，因此报告为 `warn` 但不阻断；5 分钟空闲存活与资源点样本已闭环，冷启动首帧、连续漂移和同机竞品对比仍未闭环。

2026-07-15 的 schema v3 运行时证据 [CI run 29349078110](https://github.com/TheLastSheep/atools/actions/runs/29349078110) 全部成功。20 个全新 release 进程均完成主窗口独占的 release smoke、脱敏复制 TaskRun 校验和内置计算器首次激活；插件激活 P99 为 144 ms，20 轮 RSS P99 为 114.063 MiB，首个前端交互报告 P99 为 5469 ms。第 21 个进程在完整 smoke 后继续存活 300 秒，空闲 RSS 点样本为 111.766 MiB；`.app` 目录为 32.74 MiB。P50/P95/P99、各轮原始值、CPU 点样本与运行配置保存在 `macos-runtime-performance-32ddd067...` artifact。该结果证明内置轻量插件的可复现冷启动边界，不替代重型第三方插件、插件内存增量、真实冷启动首帧或同机竞品数据。

## 2026-07-24 本机原生验收基线

本轮在 Apple M5、32 GiB、arm64 MacBook Pro 上使用本地 release `.app` 连续执行 20 轮原生 smoke，并在第 21 个进程完成 smoke 后保持空闲 300 秒。报告状态为 `pass`，原始证据保存在 `artifacts/performance/macos-runtime-local-20260724.json`；搜索规模证据保存在 `artifacts/performance/search-index-local-20260724.json`。

| 指标 | P50 | P95 | P99 / 稳定点 |
| --- | ---: | ---: | ---: |
| 启动到进程出现 | 107 ms | 189 ms | 189 ms |
| 启动到首份前端交互报告 | 600 ms | 653 ms | 653 ms |
| Option+Z 显示路径 | 7.057 ms | 8.086 ms | 8.086 ms |
| 原生搜索 | 1 ms | 24 ms | 24 ms |
| 内置插件激活 | 25 ms | 43 ms | 43 ms |
| smoke 完成时 RSS | 130.563 MiB | 133.938 MiB | 133.938 MiB |
| 5 分钟稳定空闲 | — | — | 111.125 MiB / 0% CPU |

本地 `.app` 为 37,109,312 bytes（约 35.39 MiB），主可执行文件为 36,675,664 bytes（约 34.98 MiB）。10k/100k 搜索报告的最慢 P99 为 6.926 ms，低于 80 ms 回退门槛。所有本地性能阈值均通过；首份前端报告仍是自动化可交互代理值，不等同于重启机器后的真正冷启动首帧。当前报告也没有同时取得 uTools/ZTools 的完整同机同场景矩阵，因此不得据此宣称 ATools 已证明全面快于竞品。
