...
| 安装包体积 | 验证中 | `runtime-performance-evidence` 将 unsigned release `.app` 目录与主可执行文件原始字节数写入 JSON artifact；首次远程运行尚待完成，DMG 与同机竞品仍待测 |
| 冷启动到可交互 | 部分完成 | 新基准连续创建 20 个进程并记录首份前端 release-smoke 报告；这是可交互代理，不是重启机器后的冷启动首帧 |
| 热键到首帧 | 未完成 | smoke 证明热键注册/替换，未测按键到 UI 首帧 |
| 空闲 RSS/CPU | 部分完成 | 新基准记录每次完整 release-smoke 时的 RSS/CPU 点样本；尚缺启动稳定 5 分钟后的空闲采样 |
...
1. 让远程 macOS runtime job 产出首份包体、启动代理和 RSS/CPU 报告，再补真实冷启动、稳定空闲、插件激活和热键首帧探针。
