...
| 人与 Agent 共用 TaskRun | 验证中 | MCP/Agent 工具由 `call_tool_with_task_run` 统一；插件激活、网页快开、URL 快开、本地启动/应用、粘贴工具、路径文本操作和纯文本复制均创建 TaskRun；`copy_text` 只持久化字符数/字节数和脱敏标记，release smoke 会验证正文未进入 input | 完整远程 CI 待完成；仅切换面板属于 UI 导航，不记作任务 |
...
2. 配置签名/公证 Secrets，创建 `v3.0.0`，让发布工作流完成双架构、Gatekeeper、stapling、更新清单和正式 Release 提升。
