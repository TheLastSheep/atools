...
| 统一 Capability 目录 | 验证中 | `Capability` 核心合同与注册表统一适配内置/插件 Tool、插件 Feature 和 Skill，桌面 `list_capabilities` 与 MCP `atools://capabilities` 共用目录；完整远程 CI 待完成 | 外部 MCP 执行器尚未接入；本地应用启动项属于搜索数据，不伪装成可复用 Tool |
...
1. 在现有 runtime 报告上补真实冷启动、稳定空闲、插件激活和热键首帧探针。
2. 为纯文本复制设计不落敏感正文的 TaskRun/审计合同。
3. 配置签名/公证 Secrets，创建 `v3.0.0`，让发布工作流完成双架构、Gatekeeper、stapling、更新清单和正式 Release 提升。
