...
- [x] 空搜索且没有固定指令时不渲染固定空态，首页直接进入最近使用。
- [x] `设置`、`插件管理`、`导入 ZTools 插件`、`Agent / MCP` 通过搜索结果进入，不占用默认首页首层空间。
- [x] `固定` 分区最多显示 `固定栏显示行数 * 9` 个固定指令，超出部分不挤占最近使用区域。
- [x] 最近使用磁贴优先显示插件原图标；无图标时显示来源类型 SVG fallback，图标外层约 32px，且网格无横向溢出。
- [x] 首页一级展示结果中心、插件生态、Agent / MCP 与 Pasteboard，并显示桌面运行状态。
- [x] 空搜索首页窗口高度保持在 280–340px，并随固定/最近分区自适应，不保留透明死区。
...
- [x] 设置页在 800px 主窗下使用约 58px 顶栏、44px 标签和 42px Z 图标，更多按钮三点居中，且没有横向溢出。
...
- [x] 设置页以紧凑设置窗口展开，目标高度约 600px；在低视口下按 `min(100vh, 600px)` 收敛，不出现裁切或横向溢出。
- [x] 设置页在 800px 主窗下使用约 184px 侧栏、48px 菜单行、18px 图标和 13px 字号，且没有横向溢出。
- [x] 设置页在 800px 主窗下使用约 20px 内容 padding、15px 分组标题和 76px 设置行，且没有横向溢出。
- [x] 设置页在 800px 主窗下使用约 40px 高输入/下拉、36px 按钮和 52x30px 开关，且没有横向溢出。
...
- [x] 唤醒黑名单通过 AppKit 原生读取 macOS 前台应用，不再启动 `osascript` 或依赖 Automation 权限。
